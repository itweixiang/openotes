...
- rehash机制是怎样的？
  ...
  redis中使用dict数据结构来记录kv的关系。rehash的过程中，需要将原先dict中的数据，迁移到新的dict中，数据量比较大时，rehash耗时会很长。

  但是，redis是渐进式rehash，分多次进行迁移，旧的dict迁移到新的dict之后，就会把就的值删掉。数据写入时，直接在新的dict写。数据读取时，先读旧的dict，没有再读新的dict
...
- 如何保证双写一致性？
  ...
  追求强一致性，那么就得在写入时加锁，在锁中写库和写缓存。

  只要求最终一致性，那么将缓存时间设置稍微短一点。在数据写入成功之后，再删除缓存。
